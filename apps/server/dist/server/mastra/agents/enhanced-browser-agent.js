 ...
 */
import { configUtils, globalConfig } from './config/enhanced-config.js';
import { errorHandler } from './error/error-handler.js';
import { browserAgent, contextManager, enhancedWrapper, toolManager, } from './modules/browser-agent.js';
// 简化的日志记录
const logger = {
...
export class EnhancedBrowserAgent {
    ...
    async executeOperation(operation, target, options) {
        try {
            ...
            // 使用错误处理器进行智能恢复
            const recoveryResult = await errorHandler.handleError(error instanceof Error ? error : new Error(String(error)), operation, target || '', options || {}, 0);
    ...
    async getPageCapabilities() {
        ...
        try {
            ...
            if (toolNames.some((name) => name.includes('tap') ||
                name.includes('Tap') ||
                name.includes('click'))) {
...
// 导出所有核心组件
export { browserAgent, contextManager, enhancedWrapper, toolManager, errorHandler, globalConfig, };
