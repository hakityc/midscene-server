 ...
 * 借鉴 Midscene 的错误处理机制
 */
// import { StrategySelector } from '../strategies/intelligent-strategies.js'; // 未使用，暂时注释
// 简化的日志记录
const logger = {
...
export class ErrorHandler {
     ...
     * 处理操作错误
     */
    async handleError(error, operationType, operationTarget, operationArgs, retryCount = 0) {
        const errorContext = {
            operationType,
            operationTarget,
            operationArgs,
     ...
     * 处理元素定位失败
     */
    async handleElementLocationFailure(error) {
        const actions = [
            '等待页面稳定',
            '重新获取页面上下文',
            '使用更具体的定位描述',
     ...
     * 处理超时失败
     */
    async handleTimeoutFailure(error) {
     ...
     * 处理页面状态失败
     */
    async handlePageStateFailure(error) {
        if (error.retryCount === 0) {
            return {
                success: true,
     ...
     * 处理通用失败
     */
    async handleGenericFailure(error) {
        return {
            success: true,
