 ...
 * @returns 格式化后的用户友好消息
 */
export function formatTaskTip(rawTip) {
 ...
 * 从模式中确定任务类别
 */
function getCategoryFromPattern(pattern) {
    const patternStr = pattern.source.toLowerCase();
    if (patternStr.includes('planning'))
        return 'planning';
    if (patternStr.includes('insight') || patternStr.includes('locate'))
        return 'insight';
    if (patternStr.includes('action'))
        return 'action';
    if (patternStr.includes('assert') || patternStr.includes('verify'))
        return 'verify';
    if (patternStr.includes('extract'))
        return 'extract';
    if (patternStr.includes('error'))
        return 'error';
    if (patternStr.includes('retry'))
        return 'retry';
    if (patternStr.includes('complete'))
        return 'complete';
    return 'general';
}
/**
 * 获取任务进度阶段描述
 */
export function getTaskStageDescription(category) {
