...
// AI 请求处理器
export function createAgentExecuteHandler() {
        ...
        try {
            ...
            await browserAgent.streamVNext(params, {
                ...
                onFinish: ({ steps, text, finishReason }) => {
                    ...
                    const response = createSuccessResponse(message, `Agent 处理完成`);
                    send(response);
                },
            });
        }
            ...
            const response = createErrorResponse(message, error, 'Agent 处理失败');
            send(response);
        }
    };
}
