...
// AI 请求处理器
export function createDownloadVideoHandler() {
    return async ({ connectionId, send }, message) => {
        ...
        try {
            ...
            const originalUrl = message.payload?.params ?? message.content?.body;
            // 标准化抖音链接
            ...
            await fetch('https://meishijiao.cn/api/v1/parse?url=' +
                ...
                if (videoUrl) {
                    send(createSuccessResponse(message, videoUrl, WebSocketAction.DOWNLOAD_VIDEO_CALLBACK));
                }
                else {
