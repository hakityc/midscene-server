...
import { setBrowserConnected } from "../routes/health.js";
export class OperateService extends EventEmitter {
    ...
    isStopping = false; // 标志服务正在停止，防止重连
    ...
    setupTaskStartTipCallback() {
        ...
        // 设置新的回调，同时保留原有功能
        this.agent.onTaskStartTip = async (tip) => {
     ...
     * 确保连接当前标签页 - 在所有操作前调用
     */
    async ensureCurrentTabConnection() {
        try {
            // 先确保服务已初始化
     ...
     */
    async runWithRetry(_prompt, maxRetries, singleAttemptRunner) {
        let lastError = null;
        for (let attempt = 1; attempt <= maxRetries; attempt++) {
            try {
    ...
    isConnectionError(error) {
        ...
        return (errorMessage.includes("Debugger is not attached") ||
     ...
     * 执行 AI 任务
     */
    async execute(prompt, maxRetries = 3) {
        // 如果服务未启动，自动启动
        if (!this.isStarted()) {
    ...
    async executeWithRetry(prompt, _attempt, _maxRetries) {
     ...
     * 执行 AI 断言
     */
    async expect(prompt, maxRetries = 3) {
        // 如果服务未启动，自动启动
        if (!this.isStarted()) {
            console.log("🔄 服务未启动，自动启动 OperateService...");
            await this.start();
    ...
    async expectWithRetry(prompt, _attempt, _maxRetries) {
        // 此时应该已经确保服务启动，如果仍然没有agent，说明启动失败
        if (!this.agent) {
     ...
     * 执行 YAML 脚本
     */
    async executeScript(prompt, maxRetries = 3, originalCmd) {
        // 如果服务未启动，自动启动
        if (!this.isStarted()) {
            console.log("🔄 服务未启动，自动启动 OperateService...");
            await this.start();
    ...
    async executeScriptWithRetry(prompt, _originalCmd, _attempt, _maxRetries) {
        // 此时应该已经确保服务启动，如果仍然没有agent，说明启动失败
        if (!this.agent) {
     ...
     */
    async evaluateJavaScript(script, originalCmd) {
        try {
            // 如果服务未启动，自动启动
            if (!this.isStarted()) {
                console.log("🔄 服务未启动，自动启动 OperateService...");
