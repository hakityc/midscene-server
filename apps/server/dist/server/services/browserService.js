...
function extractMCPErrorDetails(error) {
    ...
    if (errorObj && typeof errorObj === 'object') {
        const errorAny = errorObj;
        if (errorAny.code)
            mcpErrorInfo.errorCode = errorAny.code;
        if (errorAny.args)
            mcpErrorInfo.toolArgs = errorAny.args;
        if (errorAny.model)
            mcpErrorInfo.model = errorAny.model;
    }
    return mcpErrorInfo;
}
// 浏览器服务类
export class BrowserService {
    ...
    browserAgent = mastra.getAgent('browserAgent');
    /**
     * 执行浏览器任务
     * @param prompt 用户输入的提示词
     * @returns 任务执行结果
     */
    async executeBrowserTask(prompt) {
        ...
        try {
            ...
            try {
                ...
                this.logger.error('❌ 流式响应处理过程中发生错误', {
                    streamError: streamError instanceof Error
                        ? streamError.message
                        : String(streamError),
