...
import { OperateService } from './operateService.js';
export class TaskService {
    taskAgent = mastra.getAgent('taskAgent');
    operateService;
    constructor() {
     ...
     * @returns 完整的文本响应
     */
    async extractTextFromStream(response) {
        let fullResponse = '';
    ...
    parseTaskSteps(textResponse) {
        ...
        try {
            ...
            if (jsonMatch) {
                ...
                // 验证数组中的每个元素是否包含必要的字段
                const isValid = parsed.every((step) => typeof step === 'object' &&
                    step !== null &&
                    'action' in step &&
                    'verify' in step &&
    ...
    async execute(prompt) {
        try {
            ...
            try {
                ...
                throw new Error(connectError instanceof Error
