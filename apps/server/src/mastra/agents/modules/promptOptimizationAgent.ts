...
### 示例 2：结合用户描述优化（用户上传了截图并描述）
用户：点击登录按钮。我上传的截图显示页面右上角有一个蓝色的"登录"按钮，旁边还有一个灰色的"注册"按钮
动作类型：aiTap
回答：点击页面右上角的蓝色"登录"按钮（位于灰色"注册"按钮的左侧）

### 示例 3：结合用户描述优化断言
用户：检查搜索结果。截图中显示页面中间有一个搜索结果列表，每条结果包含标题、摘要和时间戳
动作类型：aiAssert
回答：
...
1. **理解用户意图**：分析用户想要操作/验证的目标
2. **提取关键信息**：从用户的描述中提取元素的位置、颜色、文字、图标等特征
3. **补充缺失信息**：根据动作类型的要求，建议添加缺失的定位信息
