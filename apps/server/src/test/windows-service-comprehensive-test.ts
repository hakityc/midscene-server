...
async function test1_ServiceLifecycle() {
  ...
  try {
    ...
    log(
      `  启动后 - isStarted: ${afterStartStarted}, isReady: ${afterStartReady}`,
    );
    ...
    log(
      `  停止后 - isStarted: ${afterStopStarted}, isReady: ${afterStopReady}`,
    );
...
async function test2_DeviceInfoAndScreenshot() {
  ...
  try {
    ...
    // 验证截图格式（应该是 base64）
    const isBase64 =
      /^data:image\/(png|jpeg);base64,/.test(screenshot) ||
...
async function test5_AITaskExecution() {
  ...
  if (!TEST_CONFIG.enableAITasks) {
    ...
    log(
      '提示: 设置 TEST_CONFIG.enableAITasks = true 并连接 Windows 客户端来运行此测试',
    );
    return true;
  }
...
async function test8_ConcurrentOperations() {
  ...
  try {
    ...
    // 验证结果一致性
    if (
      results[0].width !== results[1].width ||
      results[1].width !== results[2].width
    ) {
