...
async function quickTest() {
  ...
  try {
    ...
    console.log(
      `  ✓ 屏幕尺寸: ${size.width}x${size.height} (DPR: ${size.dpr || 1})`,
    );
    ...
    console.log(
      `  ✓ 支持 ${actions.length} 个动作: ${actions.map((a) => a.name).join(', ')}`,
    );
  ...
  try {
    ...
    console.log(
      `  ✓ Agent 状态: launched=${status.isLaunched}, destroyed=${status.isDestroyed}`,
    );
  ...
  if (testsFailed > 0) {
    ...
    console.log(
      '  - 测试 AI 功能: 需要连接 Windows 客户端并启用 enableAITasks',
    );
    process.exit(0);
  }
}
