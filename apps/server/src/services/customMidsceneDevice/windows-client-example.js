...
class WindowsClient {
  ...
  loadDependencies() {
    try {
      ...
      console.log('💡 请运行: npm install ws robotjs screenshot-desktop clipboardy node-window-manager')
    }
  }
  ...
  async handleMessage(data) {
    try {
      ...
      console.log(`📨 收到消息: ${message.type} - ${message.action || message.requestId || ''}`)
  ...
  async handleRequest(request) {
    ...
    try {
      ...
      switch (action) {
        ...
        case 'mouseDrag':
          result = await this.mouseDrag(params.fromX, params.fromY, params.toX, params.toY)
          break
  ...
  reconnect() {
    ...
    console.log(`🔄 ${CONFIG.reconnectInterval / 1000}秒后重连 (尝试 ${this.reconnectAttempts}/${CONFIG.maxReconnectAttempts})`)
