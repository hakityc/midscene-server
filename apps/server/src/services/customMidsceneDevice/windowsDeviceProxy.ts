...
`;
	...
	async destroy(): Promise<void> {
		...
		if (this.options.debug) {
			console.log(`🛑 WindowsDeviceProxy destroyed: ${this.options.deviceName}`);
		}
	 ...
	 */
	describe(): string {
		return this.description || `Windows Device Proxy - ${this.options.deviceName}`;
	}
	...
	private async keyPress(key: string, modifiers?: string[]): Promise<void> {
		...
		if (this.options.debug) {
			console.log(`⌨️ Press key: ${key}${modifiers ? ` (${modifiers.join("+")})` : ""}`);
		}
