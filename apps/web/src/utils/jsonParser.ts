...
export function parseJsonToForm(jsonMessage: WsInboundMessage) {
  ...
  switch (action) {
    ...
    case 'connectWindow': {
      ...
      if (params) {
        result.connectWindowId = params.windowId ? String(params.windowId) : '';
    ...
    case 'summarize': {
      ...
      if (params) {
        result.summarizeFullPage =
          params.fullPage !== undefined ? params.fullPage : true;
