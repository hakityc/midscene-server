...
async function debugAgentError() {
  ...
  try {
    ...
    console.log(
      ...
      process.env.TASK_MIDSCENE_MODEL_NAME ? '✓ 已设置' : '❌ 未设置',
    );
    console.log(
      'TASK_OPENAI_API_KEY:',
      process.env.TASK_OPENAI_API_KEY ? '✓ 已设置' : '❌ 未设置',
    );
    console.log(
      'TASK_OPENAI_BASE_URL:',
      process.env.TASK_OPENAI_BASE_URL ? '✓ 已设置' : '❌ 未设置',
    );
    console.log('');
    ...
    try {
      ...
      console.log(
        'Agent 描述:',
        browserAgent.description?.substring(0, 100) + '...',
      );
    } catch (agentError) {
    ...
    try {
      ...
      console.log(
        '工具列表:',
        Object.keys(tools).slice(0, 5).join(', ') +
          (Object.keys(tools).length > 5 ? '...' : ''),
      );
    } catch (toolsError) {
