...
import { WebSocketErrorHandler } from './handlers/errorHandler';
...
export const setupWebSocket = (app: Hono) => {
  ...
  const dispatchMessage = async (
    ...
    if (!handler) {
      WebSocketErrorHandler.handleUnknownAction(ws, message, message.content.action);
      return;
    }
  ...
  app.get(
    ...
    upgradeWebSocket((c) => {
      ...
      return {
        ...
        onMessage(event, ws) {
          try {
            ...
            dispatchMessage(connectionId, message, ws).catch((error) => {
              WebSocketErrorHandler.handleMessageProcessingError(ws, message, error, {
                connectionId,
                action: message.content.action,
              });
            });
          } catch (error) {
            WebSocketErrorHandler.handleParseError(ws, error, event.data.toString(), connectionId);
          }
        },
        ...
        onError(error: any) {
          WebSocketErrorHandler.handleConnectionError(connectionId, error);
        },
      };
    })
  );
  ...
  app.post('/ws/broadcast', async (c) => {
    try {
      ...
      let sentCount = 0;
      const broadcastMessage = MessageBuilder.createBroadcastMessage(message, conversationId);
