...
### 🔧 基础操作 API
- **导航页面**: \`midscene_navigate(url)\` - 导航到指定 URL
...
**推荐的结构化执行方式**:
\`\`\`
我将为您打开抖音网站并搜索播放哪吒2的视频。

[调用 midscene_navigate 工具：导航到 "https://www.douyin.com"]
...
**为什么这样做更好**：
- ✅ 使用 \`midscene_navigate\` 而不是 \`aiAction\` 进行页面导航
...
### 📍 页面导航任务
- **最佳策略**:
  - 使用 \`midscene_navigate(url)\` 进行页面导航
...
**模式1: 导航 + 操作**
\`\`\`
midscene_navigate() → aiWaitFor() → aiTap() → aiInput()
\`\`\`
...
**应该这样做**：
- ✅ \`midscene_navigate(url)\` + \`aiWaitFor(element)\`
- ✅ 每个操作前都确保元素可见
- ✅ 关键步骤后使用 \`aiAssert()\` 验证
- ✅ 优先使用具体的 API 方法
