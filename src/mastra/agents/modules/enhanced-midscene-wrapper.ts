...
export class EnhancedMidsceneWrapper {
   ...
   * 通用 MCP 工具调用方法
   */
  private async callMcpTool(toolName: string, _args: any): Promise<any> {
    try {
   ...
   * 增强定位提示词
   */
  private enhanceLocationPrompt(prompt: string, strategy: string): string {
    const enhancements = {
   ...
   * 增强查询提示词
   */
  private enhanceQueryPrompt(prompt: string): string {
   ...
   * 记录操作历史
   */
  private recordOperation(type: string, result: OperationResult): void {
