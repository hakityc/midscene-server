...
export class ContextManager {
  ...
  constructor(memory: Memory, sessionId?: string) {
    this.memory = memory;
    this.currentSession = {
   ...
   * 更新页面上下文
   */
  async updatePageContext(context: Partial<PageContext>): Promise<void> {
    try {
   ...
   */
  async freezeContext(): Promise<void> {
   ...
   * 解冻页面上下文
   */
  async unfreezeContext(): Promise<void> {
   ...
   */
  getCurrentPageContext(): PageContext | undefined {
   ...
   * 获取页面上下文缓存
   */
  getCachedPageContext(url: string): PageContext | undefined {
   ...
   * 获取智能操作建议
   */
  async getOperationSuggestions(taskType: string): Promise<any> {
   ...
   * 保存上下文到记忆系统
   */
  private async saveContextToMemory(context: PageContext): Promise<void> {
    try {
   ...
   * 生成学习洞察
   */
  private generateLearningInsights(successfulOps: OperationRecord[]): any {
