...
export const midsceneQueryContentTool = createTool({
  ...
  inputSchema: z.object({
    prompt: z.string().describe('查询提示，如"获取所有商品价格"、"查找联系信息"等')
  }),
  outputSchema: z.object({
    success: z.boolean().describe('操作是否成功'),
...
export const midsceneAssertStateTool = createTool({
  ...
  inputSchema: z.object({
    assertion: z.string().describe('要验证的条件，如"页面已加载完成"、"登录成功"等'),
...
export const midsceneWaitForTool = createTool({
  ...
  inputSchema: z.object({
    condition: z.string().describe('要等待的条件，如"搜索结果出现"、"表单加载完成"等'),
