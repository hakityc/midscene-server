...
// 执行断言
await webService.expect('页面标题是"首页"');
...
// 执行 AI 任务
await windowsService.execute('点击开始菜单');

// 获取窗口列表
const windows = await windowsService.getWindowList();

// 连接窗口
await windowsService.connectWindow({ windowTitle: '记事本' });

// 停止服务
await windowsService.stop();
```

### 使用重构后的 Action Handlers
...
export class WebOperateService extends EventEmitter {
  ...
  public createTaskTipCallback<T>(config: { /* 9 个参数 */ }): ... { ... }
...
#### 重构后（WebOperateServiceRefactored）
...
```typescript
// ✅ 统一的枚举（BaseOperateService.ts）
export enum OperateServiceState {
  STOPPED = 'stopped',
  STARTING = 'starting',
  RUNNING = 'running',
  STOPPING = 'stopping',
  RECONNECTING = 'reconnecting',  // Windows 可以忽略
}
...
public createTaskTipCallback<T>(
  ...
  // formatTaskTip 和 getTaskStageDescription 直接导入使用
  // 不再作为参数传入
...
// ❌ windows/execute.ts - 118 行（几乎完全相同）
...
```typescript
// ✅ actionHandlerFactory.ts - 80 行（通用工厂函数）
...
// ✅ exampleUsage.ts - Windows 端只需 5 行
export function createWindowsAiHandler(): MessageHandler {
  return createAiHandlerFactory(
...
- Web Handler：115 行 → 5 行（**减少 95.7%**）
- Windows Handler：118 行 → 5 行（**减少 95.8%**）
- 总代码：233 行 → 90 行（**减少 61.4%**）
- 消除了 143 行重复代码
...
1. **创建基类和重构后的服务**
   - 创建 `BaseOperateService.ts`
   - 创建 `WebOperateServiceRefactored.ts`
   - 创建 `WindowsOperateServiceRefactored.ts`
   - 创建 `actionHandlerFactory.ts`

2. **添加测试**
   - 创建 `BaseOperateService.test.ts`
   - 运行测试确保新代码正常工作

3. **逐步迁移 Action Handlers**
   - 创建新的 Handler（基于工厂函数）
   - 保留旧的 Handler 作为备份
