# 开发规范 - 快速参考

> 完整版规范请查看：[开发规范.md](./开发规范.md)

---

## 🎯 核心原则

1. **代码质量优先**：使用 Biome 保持代码整洁
2. **类型安全**：避免使用 `any`，充分利用 TypeScript
3. **单一职责**：每个函数/类只做一件事
4. **DRY 原则**：不要重复自己（Don't Repeat Yourself）
5. **测试驱动**：关键功能必须有测试覆盖

---

## 📝 代码风格速查

### 格式化
```bash
# 自动格式化代码
pnpm format

# 检查代码质量
pnpm check

# 自动修复问题
pnpm fix
```

### 代码规则
- ✅ 单引号 `'string'`
- ✅ 2 空格缩进
- ✅ 行宽 80 字符
- ✅ 使用 LF 换行符

---

## 🏷️ 命名速查

| 类型 | 规则 | 示例 |
|------|------|------|
| 文件 | camelCase | `webOperateService.ts` |
| 组件 | PascalCase | `JsonPreview.tsx` |
| 类 | PascalCase | `BaseOperateService` |
| 接口 | PascalCase | `WebSocketMessage` |
| 函数 | camelCase | `handleWebSocketMessage()` |
| 变量 | camelCase | `userName`, `isConnected` |
| 常量 | UPPER_SNAKE_CASE | `MAX_RETRY_COUNT` |
| 枚举 | PascalCase | `OperateServiceState` |
| 枚举成员 | UPPER_CASE | `STOPPED`, `RUNNING` |

---

## 🏗️ 架构速查

### 设计模式

#### 1. 单例模式（Service 类必须）
```typescript
export class MyService {
  private static instance: MyService | null = null;

  private constructor() { }

  public static getInstance(): MyService {
    if (!MyService.instance) {
      MyService.instance = new MyService();
    }
    return MyService.instance;
  }
}
```

#### 2. 工厂模式（Handler 创建）
```typescript
export const handleExecute = createAiHandlerFactory(
  () => WebOperateServiceRefactored.getInstance(),
  'Web',
  { checkAndReconnect: true }
);
```

#### 3. 模板方法模式（Service 继承）
```typescript
export abstract class BaseOperateService<TAgent> {
  protected abstract createAgent(): Promise<void>;
  protected abstract initializeConnection(): Promise<void>;
}
```

---

## 📊 类型定义速查

### 基本原则
- ✅ 优先使用 `interface` 定义对象
- ✅ 使用 `type` 定义联合类型、交叉类型
- ✅ 避免 `any`，必须使用时添加注释
- ✅ 使用泛型增强复用性

### 示例
```typescript
// Interface
export interface WebSocketMessage {
  meta: { messageId: string; };
  payload: { action: string; };
}

// Type Alias
export type ClientType = 'web' | 'windows';

// Generic
export class BaseService<TAgent extends AgentType> {
  public agent: TAgent | null = null;
}
```

---

## 🚨 错误处理速查

```typescript
// 1. 自定义错误类
export class AppError extends Error {
  constructor(
    public message: string,
    public statusCode: number = 500,
  ) {
    super(message);
  }
}

// 2. Try-Catch-Finally
try {
  await operation();
} catch (error) {
  logger.error({ error }, '操作失败');
  throw new AppError('操作失败', 500);
} finally {
  await cleanup();
}

// 3. WebSocket 错误响应
const errorResponse = createErrorResponse(
  message,
  error,
  '操作失败'
);
send(errorResponse);
```

---

## 📝 日志速查

### 日志级别
| 级别 | 使用场景 |
|------|---------|
| `debug` | 调试信息（开发环境） |
| `info` | 一般信息、关键操作 |
| `warn` | 警告、潜在问题 |
| `error` | 错误信息 |
| `fatal` | 严重错误 |

### 结构化日志
```typescript
// ✅ 好的日志
serviceLogger.info(
  {
    taskId,
    duration,
    params,
  },
  '任务执行成功'
);

// ❌ 不好的日志
logger.info('任务成功');  // 缺少上下文
```

### 专用 Logger
```typescript
import {
  wsLogger,      // WebSocket
  serverLogger,  // 服务器
  serviceLogger, // 服务
} from '@/utils/logger';
```

---

## 🔌 WebSocket 速查

### 消息结构
```typescript
// 入站（客户端 -> 服务端）
interface WsInboundMessage {
  meta: {
    messageId: string;
    conversationId: string;
    timestamp: number;
    clientType?: 'web' | 'windows';
  };
  payload: {
    action: WebSocketAction;
    params: any;
  };
}

// 出站（服务端 -> 客户端）
interface WsOutboundMessage {
  meta: { messageId, conversationId, timestamp };
  payload: {
    action: WebSocketAction;
    status: 'success' | 'failed';
    result?: any;
    error?: string;
  };
}
```

### Handler 模板
```typescript
export const handleAction: MessageHandler = async (
  { connectionId, send },
  message
) => {
  setLogContextFromMessage(message, connectionId);

  try {
    const result = await processAction(message.payload.params);
    send(createSuccessResponse(message, result));
  } catch (error) {
    send(createErrorResponse(message, error, '操作失败'));
  }
};
```

---

## 🧪 测试速查

### 测试文件位置
```
src/services/MyService.ts
src/services/__tests__/MyService.test.ts
```

### 测试命令
```bash
pnpm test              # 运行所有测试
pnpm test:coverage     # 查看覆盖率
pnpm test:ui           # 测试 UI
pnpm test:watch        # 监听模式
```

### 测试模板
```typescript
describe('MyService', () => {
  let service: MyService;

  beforeEach(() => {
    service = MyService.getInstance();
  });

  afterEach(() => {
    MyService.resetInstance();
  });

  it('应该正确初始化', () => {
    expect(service.getState()).toBe('stopped');
  });
});
```

---

## 📚 文档速查

### 何时编写文档
✅ **必须写**：
- 大型重构
- 新功能开发
- 高价值问题修复
- 性能优化

❌ **不需要**：
- 简单 Bug 修复
- 代码格式调整
- 依赖升级

### 文档分类
```
docs/
├── 功能开发/
├── 架构重构/
├── 问题修复/
└── 性能优化/
```

### 文档命名
- ✅ 使用中文名称
- ✅ 简洁明了（< 20 字）
- ✅ 包含关键词

---

## 🔄 Git 提交速查

### 提交格式
```
<type>(<scope>): <subject>

<body>
```

### Type 类型
| Type | 说明 | 示例 |
|------|------|------|
| `feat` | 新功能 | `feat(service): 添加缓存机制` |
| `fix` | Bug 修复 | `fix(websocket): 修复连接问题` |
| `refactor` | 重构 | `refactor(service): 提取基类` |
| `docs` | 文档 | `docs: 更新开发规范` |
| `test` | 测试 | `test: 添加单元测试` |
| `chore` | 构建/工具 | `chore: 升级依赖` |
| `perf` | 性能 | `perf: 优化查询性能` |

### 示例
```bash
git commit -m "feat(service): 添加 BaseOperateService 基类"
git commit -m "fix(handler): 修复消息 ID 不匹配问题"
git commit -m "refactor(websocket): 使用工厂模式"
```

---

## 🚀 Service 开发速查

### Service 类模板
```typescript
export class MyService extends BaseOperateService<MyAgent> {
  private static instance: MyService | null = null;

  private constructor() {
    super();
  }

  public static getInstance(): MyService {
    if (!MyService.instance) {
      MyService.instance = new MyService();
    }
    return MyService.instance;
  }

  public static resetInstance(): void {
    MyService.instance = null;
  }

  protected async createAgent(): Promise<void> {
    // 实现
  }

  protected async initializeConnection(): Promise<void> {
    // 实现
  }

  protected getServiceName(): string {
    return 'MyService';
  }
}
```

---

## 📦 Import 顺序

```typescript
// 1. Node.js 内置
import { EventEmitter } from 'node:events';

// 2. 第三方依赖
import { Hono } from 'hono';

// 3. 内部模块（别名）
import type { WebSocketMessage } from '@/types';
import { logger } from '@/utils/logger';

// 4. 相对路径
import { helper } from './helper';
```

---

## ⚡ 性能优化速查

```typescript
// ✅ 并行执行
const [r1, r2, r3] = await Promise.all([
  fetch1(),
  fetch2(),
  fetch3(),
]);

// ✅ 缓存机制
private cache = new Map<string, any>();

// ✅ 资源清理
try {
  await operation();
} finally {
  await cleanup();
}

// ✅ 性能监控
const start = performance.now();
await operation();
const duration = performance.now() - start;
logger.info({ duration }, '操作完成');
```

---

## 🔒 安全速查

```typescript
// ✅ 环境变量
const apiKey = process.env.API_KEY;

// ✅ 输入验证
if (typeof input !== 'string') {
  throw new AppError('Invalid input', 400);
}

// ✅ 日志脱敏
function sanitize(data: any) {
  return {
    ...data,
    password: '***',
    token: '***',
  };
}
```

---

## 📋 常用命令

```bash
# 开发
pnpm dev
pnpm --filter @midscene/server dev

# 构建
pnpm build
pnpm build:prod
pnpm build:staging

# 测试
pnpm test
pnpm test:coverage
pnpm test:ui

# 代码质量
pnpm lint
pnpm format
pnpm check
pnpm fix
```

---

## 📖 相关文档

- [完整开发规范](./开发规范.md)
- [服务层重构方案](./架构重构/服务层重构方案.md)
- [代码对比与迁移指南](./架构重构/代码对比与迁移指南.md)
- [WebSocket 传值说明](./WebSocket传值说明.md)
- [Monorepo 使用指南](./Monorepo使用指南.md)

---

**提示**：本文档是快速参考版本，详细规范请查看完整版文档。

