# Midscene Server 项目开发规范

> **版本**: v1.0.0  
> **更新日期**: 2025年10月24日  
> **适用范围**: midscene-server Monorepo

---

## 📋 目录

- [1. 代码风格与格式化](#1-代码风格与格式化)
- [2. 命名规范](#2-命名规范)
- [3. 架构与设计模式](#3-架构与设计模式)
- [4. 类型定义规范](#4-类型定义规范)
- [5. 错误处理规范](#5-错误处理规范)
- [6. 日志规范](#6-日志规范)
- [7. WebSocket 通信规范](#7-websocket-通信规范)
- [8. 测试规范](#8-测试规范)
- [9. 文档规范](#9-文档规范)
- [10. Git 提交规范](#10-git-提交规范)
- [11. 服务层开发规范](#11-服务层开发规范)
- [12. 文件组织规范](#12-文件组织规范)
- [13. 性能优化规范](#13-性能优化规范)
- [14. 安全规范](#14-安全规范)

---

## 1. 代码风格与格式化

### 1.1 基本规则

项目使用 **Biome** 进行代码格式化和 lint，配置文件：`biome.json`

```javascript
// 核心配置
{
  "formatter": {
    "lineWidth": 80,
    "indentStyle": "space",
    "indentWidth": 2,
    "lineEnding": "lf"
  },
  "javascript": {
    "formatter": {
      "quoteStyle": "single"
    }
  }
}
```

### 1.2 强制规则

- ✅ **使用单引号** (`'`) 代替双引号
- ✅ **缩进**: 2 个空格
- ✅ **行宽**: 最大 80 字符
- ✅ **换行符**: LF (Unix 风格)
- ✅ **分号**: 自动添加（Biome 处理）
- ✅ **尾随逗号**: 多行对象/数组添加

### 1.3 代码组织顺序

```typescript
// 1. Import 声明（按类型分组）
import type { SomeType } from 'external-package';
import { externalFunction } from 'external-package';
import type { InternalType } from '@/types';
import { internalFunction } from '@/utils';

// 2. 类型定义
export interface MyInterface {
  // ...
}

export type MyType = string | number;

// 3. 常量定义
const MY_CONSTANT = 'value';

// 4. 函数/类定义
export class MyClass {
  // ...
}

export function myFunction() {
  // ...
}
```

### 1.4 命令

```bash
# 格式化代码
pnpm format

# 检查代码质量
pnpm check

# 自动修复问题
pnpm fix

# Lint 代码
pnpm lint
```

---

## 2. 命名规范

### 2.1 文件命名

| 类型 | 命名规则 | 示例 |
|------|---------|------|
| 普通文件 | camelCase | `webOperateService.ts` |
| 组件文件 | PascalCase | `JsonPreview.tsx` |
| 类型文件 | camelCase | `websocket.ts`, `index.ts` |
| 配置文件 | kebab-case | `tsup.config.ts` |
| 测试文件 | 原名.test.ts | `error.test.ts` |
| 工具文件 | camelCase | `logger.ts`, `error.ts` |

### 2.2 代码命名

#### 变量和函数
```typescript
// ✅ 好的命名
const userName = 'John';
const isConnected = true;
const hasError = false;
const totalCount = 10;

function getUserById(id: string) { }
function calculateTotal() { }
function handleWebSocketMessage() { }

// ❌ 不好的命名
const user_name = 'John';  // 不使用 snake_case
const connected = true;    // 布尔值应有 is/has 前缀
const t = 10;              // 变量名过短
function get(id) { }       // 函数名不明确
```

#### 类和接口
```typescript
// ✅ 类使用 PascalCase
class BaseOperateService { }
class WebOperateServiceRefactored { }
class ClientCommandHelper { }

// ✅ 接口使用 PascalCase，使用 I 前缀（可选）
interface TaskTipCallbackConfig { }
interface WebSocketMessage { }
interface IUserRepository { }  // 可选的 I 前缀

// ✅ 枚举使用 PascalCase，成员使用 UPPER_CASE
enum OperateServiceState {
  STOPPED = 'stopped',
  STARTING = 'starting',
  RUNNING = 'running',
}

enum WebSocketAction {
  EXECUTE = 'execute',
  CALLBACK_AI_STEP = 'callback_ai_step',
}
```

#### 类型别名
```typescript
// ✅ 使用 PascalCase
type AgentType = AgentOverChromeBridge | AgentOverWindows;
type TaskTipCallback = (tip: string, error?: Error) => void;
type ClientType = 'web' | 'windows';
```

#### 常量
```typescript
// ✅ 全局常量使用 UPPER_SNAKE_CASE
const MAX_RETRY_COUNT = 3;
const DEFAULT_TIMEOUT = 5000;
const API_BASE_URL = 'https://api.example.com';

// ✅ 配置对象使用 camelCase
const serverConfig = {
  port: 3000,
  host: 'localhost',
};
```

### 2.3 特殊命名约定

#### Service 类
```typescript
// 格式：<Platform>OperateService[Refactored]
class WebOperateService { }
class WebOperateServiceRefactored { }
class WindowsOperateService { }
class WindowsOperateServiceRefactored { }
```

#### Handler 函数
```typescript
// 格式：handle<Action>
function handleExecute() { }
function handleConnect() { }
function handleWebSocketMessage() { }
```

#### Logger 实例
```typescript
// 格式：<module>Logger
const wsLogger = createLogger('websocket');
const serverLogger = createLogger('server');
const serviceLogger = createLogger('service');
```

---

## 3. 架构与设计模式

### 3.1 核心架构

项目采用 **分层架构** + **设计模式** 的组合：

```
┌─────────────────────────────────────┐
│         WebSocket Layer             │
│  (handlers, builders, helpers)      │
└─────────────────────────────────────┘
              ↓
┌─────────────────────────────────────┐
│          Service Layer              │
│  (BaseOperateService + 子类)        │
└─────────────────────────────────────┘
              ↓
┌─────────────────────────────────────┐
│         Integration Layer           │
│  (Midscene, Mastra, MCP)            │
└─────────────────────────────────────┘
```

### 3.2 必须遵循的设计模式

#### 3.2.1 单例模式
所有 Service 类必须使用单例模式：

```typescript
export class MyService {
  private static instance: MyService | null = null;

  private constructor() {
    // 私有构造函数
  }

  public static getInstance(): MyService {
    if (!MyService.instance) {
      MyService.instance = new MyService();
    }
    return MyService.instance;
  }

  public static resetInstance(): void {
    MyService.instance = null;
  }
}
```

#### 3.2.2 工厂模式
创建 Handler 时使用工厂函数：

```typescript
export function createAiHandlerFactory(
  getService: () => BaseOperateService<any>,
  serviceName: string,
  options?: HandlerOptions,
): MessageHandler {
  return async ({ connectionId, send }, message) => {
    const service = getService();
    // 处理逻辑
  };
}
```

#### 3.2.3 模板方法模式
Service 基类定义算法骨架，子类实现具体步骤：

```typescript
export abstract class BaseOperateService<TAgent> {
  // 模板方法
  public async start(): Promise<void> {
    this.setState(OperateServiceState.STARTING);
    await this.createAgent();
    await this.initializeConnection();
    this.setState(OperateServiceState.RUNNING);
  }

  // 抽象方法（子类必须实现）
  protected abstract createAgent(): Promise<void>;
  protected abstract initializeConnection(): Promise<void>;
  protected abstract getServiceName(): string;
}
```

#### 3.2.4 策略模式
不同平台使用不同的执行策略：

```typescript
// Web 策略
class WebOperateServiceRefactored extends BaseOperateService {
  protected async executeAiTask(params: any) {
    // Web 特定实现
  }
}

// Windows 策略
class WindowsOperateServiceRefactored extends BaseOperateService {
  protected async executeAiTask(params: any) {
    // Windows 特定实现
  }
}
```

### 3.3 禁止的模式

- ❌ **禁止使用 God Object**：单个类不应超过 500 行
- ❌ **禁止深度嵌套**：函数嵌套不超过 4 层
- ❌ **禁止循环依赖**：模块间不允许循环引用
- ❌ **禁止全局状态污染**：使用依赖注入代替全局变量

---

## 4. 类型定义规范

### 4.1 基本原则

- ✅ **优先使用 `interface`**：对象形状定义
- ✅ **使用 `type`**：联合类型、交叉类型、基本类型别名
- ✅ **避免 `any`**：必须使用时添加注释说明原因
- ✅ **使用严格的 TypeScript 配置**

### 4.2 类型定义位置

```
src/types/
├── index.ts              # 通用类型
├── websocket.ts          # WebSocket 相关类型
├── windowsProtocol.ts    # Windows 协议类型
├── promptOptimization.ts # 提示优化类型
└── cls.ts                # 日志上下文类型
```

### 4.3 类型定义示例

#### Interface 定义
```typescript
// ✅ 清晰的接口定义
export interface WebSocketMessage {
  meta: {
    messageId: string;
    conversationId: string;
    timestamp: number;
  };
  payload: {
    action: WebSocketAction;
    params: any;
    site: string;
  };
}

// ✅ 泛型接口
export interface ApiResponse<T = any> {
  success: boolean;
  data?: T;
  message?: string;
  error?: string;
}
```

#### Type Alias
```typescript
// ✅ 联合类型
export type ClientType = 'web' | 'windows';
export type WsStatus = 'success' | 'failed';

// ✅ 函数类型
export type TaskTipCallback = (
  tip: string,
  bridgeError?: Error | null
) => void;

// ✅ 复杂类型组合
export type AgentType = AgentOverChromeBridge | AgentOverWindows;
```

#### 泛型约束
```typescript
// ✅ 使用泛型约束
export abstract class BaseOperateService<TAgent extends AgentType> {
  public agent: TAgent | null = null;
}

// ✅ 函数泛型
export function createLogger<T extends object>(
  name: string,
  meta?: T
) {
  // ...
}
```

### 4.4 避免的模式

```typescript
// ❌ 过度使用 any
function processData(data: any) { }

// ✅ 使用具体类型
function processData(data: WebSocketMessage) { }

// ❌ 不必要的类型断言
const result = getData() as string;

// ✅ 使用类型守卫
function isString(value: unknown): value is string {
  return typeof value === 'string';
}

// ❌ 隐式 any
function handleMessage(message) { }  // 参数隐式为 any

// ✅ 显式类型
function handleMessage(message: WebSocketMessage) { }
```

---

## 5. 错误处理规范

### 5.1 自定义错误类

```typescript
// ✅ 继承 Error 类
export class AppError extends Error {
  constructor(
    public message: string,
    public statusCode: number = 500,
    public isOperational: boolean = true,
  ) {
    super(message);
    Object.setPrototypeOf(this, AppError.prototype);
  }
}

// ✅ 特定领域错误
export class WebSocketError extends AppError {
  constructor(message: string, public connectionId: string) {
    super(message, 500);
  }
}
```

### 5.2 错误处理模式

```typescript
// ✅ Try-Catch-Finally
async function executeTask() {
  try {
    await someOperation();
  } catch (error) {
    // 记录错误
    serviceLogger.error({ error }, '任务执行失败');
    
    // 转换为应用错误
    if (error instanceof Error) {
      throw new AppError(error.message, 500);
    }
    
    // 未知错误
    throw new AppError('未知错误', 500);
  } finally {
    // 清理资源
    await cleanup();
  }
}

// ✅ 错误传播
async function highLevelFunction() {
  try {
    await executeTask();
  } catch (error) {
    // 添加上下文后重新抛出
    if (error instanceof AppError) {
      error.message = `高级操作失败: ${error.message}`;
      throw error;
    }
    throw error;
  }
}
```

### 5.3 错误响应

```typescript
// WebSocket 错误响应
function createErrorResponse(
  message: WebSocketMessage,
  error: Error,
  errorMessage: string,
): WsOutboundMessage {
  return {
    meta: {
      messageId: message.meta.messageId,
      conversationId: message.meta.conversationId,
      timestamp: Date.now(),
    },
    payload: {
      action: message.payload.action,
      status: 'failed',
      error: errorMessage,
    },
  };
}
```

### 5.4 错误日志

```typescript
// ✅ 记录完整的错误信息
serviceLogger.error(
  {
    error: error,
    stack: error.stack,
    context: {
      userId,
      taskId,
      timestamp: Date.now(),
    },
  },
  '任务执行失败'
);

// ❌ 不要只记录错误消息
serviceLogger.error(error.message);
```

---

## 6. 日志规范

### 6.1 日志级别

| 级别 | 使用场景 | 示例 |
|------|---------|------|
| `debug` | 调试信息，开发环境 | 变量值、函数调用 |
| `info` | 一般信息，关键操作 | 服务启动、连接建立 |
| `warn` | 警告信息，潜在问题 | 重试操作、配置缺失 |
| `error` | 错误信息，需要关注 | 操作失败、异常捕获 |
| `fatal` | 严重错误，服务不可用 | 服务崩溃、致命错误 |

### 6.2 日志格式

```typescript
// ✅ 结构化日志
wsLogger.info(
  {
    connectionId,
    messageId: message.meta.messageId,
    action: message.payload.action,
    clientType: message.meta.clientType,
  },
  '处理 WebSocket 消息'
);

// ✅ 带上下文的错误日志
serviceLogger.error(
  {
    error,
    stack: error.stack,
    taskName,
    params,
  },
  'AI 任务执行失败'
);

// ❌ 不要使用简单字符串
logger.info('处理消息');  // 缺少上下文
logger.error(error.message);  // 缺少堆栈信息
```

### 6.3 专用 Logger

```typescript
// 为不同模块创建专用 logger
export const wsLogger = createLogger('websocket');
export const serverLogger = createLogger('server');
export const serviceLogger = createLogger('service');
export const controllerLogger = createLogger('controller');

// 使用示例
wsLogger.info({ connectionId }, 'WebSocket 连接建立');
serviceLogger.debug({ agent }, '创建 Agent 实例');
```

### 6.4 日志上下文

```typescript
// ✅ 设置日志上下文（自动添加到所有日志）
setLogContextFromMessage(message, connectionId);

// ✅ 在异步操作中使用上下文
await withLogContext(
  { messageId, conversationId },
  async () => {
    // 所有日志自动包含上下文
    serviceLogger.info('开始执行任务');
  }
);
```

### 6.5 性能日志

```typescript
// ✅ 记录性能指标
const startTime = Date.now();
await executeTask();
const duration = Date.now() - startTime;

serviceLogger.info(
  {
    taskName,
    duration,
    performance: {
      startTime,
      endTime: Date.now(),
    },
  },
  `任务执行完成，耗时 ${duration}ms`
);
```

---

## 7. WebSocket 通信规范

### 7.1 消息结构

#### 入站消息（客户端 -> 服务端）
```typescript
interface WsInboundMessage<P = unknown> {
  meta: {
    messageId: string;        // 消息唯一ID
    conversationId: string;   // 会话ID
    timestamp: number;        // 时间戳
    clientType?: 'web' | 'windows';  // 客户端类型
  };
  payload: {
    action: WebSocketAction;  // 操作类型
    params: P;                // 参数
    site: string;            // 站点信息
    originalCmd: string;     // 原始命令
  };
}
```

#### 出站消息（服务端 -> 客户端）
```typescript
interface WsOutboundMessage<R = unknown> {
  meta: {
    messageId: string;
    conversationId: string;
    timestamp: number;
  };
  payload: {
    action: WebSocketAction;
    status: 'success' | 'failed';
    result?: R;
    error?: string;
  };
}
```

### 7.2 Action 定义

```typescript
// src/utils/enums.ts
export enum WebSocketAction {
  // 基础操作
  EXECUTE = 'execute',
  EXECUTE_SCRIPT = 'execute_script',
  COMMAND = 'command',
  
  // 连接管理
  CONNECT = 'connect',
  CONNECT_WINDOW = 'connect_window',
  
  // 回调
  CALLBACK_AI_STEP = 'callback_ai_step',
  
  // 其他
  DOWNLOAD_VIDEO = 'download_video',
}
```

### 7.3 Handler 实现规范

```typescript
// ✅ 使用工厂函数创建 Handler
export const handleExecute = createAiHandlerFactory(
  () => WebOperateServiceRefactored.getInstance(),
  'Web',
  { checkAndReconnect: true, supportMask: true }
);

// ✅ 自定义 Handler 模式
export const handleConnect: MessageHandler = async (
  { connectionId, send },
  message
) => {
  // 1. 设置日志上下文
  setLogContextFromMessage(message, connectionId);
  
  // 2. 记录开始日志
  wsLogger.info({ action: 'connect' }, '处理连接请求');
  
  try {
    // 3. 执行业务逻辑
    const result = await processConnect(message.payload.params);
    
    // 4. 发送成功响应
    const response = createSuccessResponse(message, result);
    send(response);
    
    // 5. 记录成功日志
    wsLogger.info({ action: 'connect' }, '连接成功');
  } catch (error) {
    // 6. 错误处理
    wsLogger.error({ error }, '连接失败');
    
    // 7. 发送错误响应
    const errorResponse = createErrorResponse(
      message,
      error as Error,
      '连接失败'
    );
    send(errorResponse);
  }
};
```

### 7.4 消息构建器

```typescript
// 使用统一的消息构建器
import {
  createSuccessResponse,
  createErrorResponse,
  createSuccessResponseWithMeta,
} from '@/websocket/builders/messageBuilder';

// ✅ 成功响应
const response = createSuccessResponse(message, result);

// ✅ 错误响应
const errorResponse = createErrorResponse(message, error, '操作失败');

// ✅ 带元数据的响应
const responseWithMeta = createSuccessResponseWithMeta(
  message,
  result,
  { duration: 1000 }
);
```

---

## 8. 测试规范

### 8.1 测试框架

项目使用 **Vitest** 进行测试，配置文件：`vitest.config.ts`

### 8.2 测试文件组织

```
src/
├── services/
│   ├── base/
│   │   ├── BaseOperateService.ts
│   │   └── __tests__/
│   │       └── BaseOperateService.test.ts
│   └── webOperateService.ts
└── utils/
    ├── error.ts
    └── __tests__/
        └── error.test.ts
```

### 8.3 测试命名

```typescript
// ✅ 使用描述性的测试名称
describe('BaseOperateService', () => {
  describe('状态管理', () => {
    it('应该正确初始化为 STOPPED 状态', () => { });
    it('当调用 start() 时应该转换到 STARTING 状态', () => { });
    it('当启动失败时应该回退到 STOPPED 状态', () => { });
  });
  
  describe('任务执行', () => {
    it('应该成功执行 AI 任务并返回结果', () => { });
    it('当 Agent 未初始化时应该抛出错误', () => { });
  });
});
```

### 8.4 测试覆盖率目标

| 类型 | 覆盖率目标 |
|------|----------|
| 核心服务类 | ≥ 85% |
| Handler 函数 | ≥ 80% |
| 工具函数 | ≥ 90% |
| 类型定义 | N/A |

### 8.5 测试命令

```bash
# 运行所有测试
pnpm test

# 运行测试并查看覆盖率
pnpm test:coverage

# 运行测试 UI
pnpm test:ui

# 监听模式
pnpm test:watch
```

### 8.6 测试示例

```typescript
import { describe, it, expect, beforeEach, afterEach } from 'vitest';
import { BaseOperateService } from '../BaseOperateService';

describe('BaseOperateService', () => {
  let service: TestOperateService;
  
  beforeEach(() => {
    service = new TestOperateService();
  });
  
  afterEach(() => {
    service.resetInstance();
  });
  
  it('应该正确初始化状态', () => {
    expect(service.getState()).toBe(OperateServiceState.STOPPED);
  });
  
  it('应该成功启动服务', async () => {
    await service.start();
    expect(service.getState()).toBe(OperateServiceState.RUNNING);
  });
});
```

---

## 9. 文档规范

### 9.1 文档分类

```
docs/
├── 功能开发/          # 新功能开发文档
├── 架构重构/          # 架构重构文档
├── 问题修复/          # Bug 修复文档
├── 性能优化/          # 性能优化文档
└── 开发规范.md        # 本文档
```

### 9.2 何时编写文档

#### ✅ 必须编写文档的场景
1. **大型重构**：涉及核心架构变更
2. **新功能开发**：独立模块或重要特性
3. **高价值问题修复**：技术难题、罕见问题
4. **性能优化**：显著的性能提升方案

#### ❌ 不需要文档的场景
1. 简单的 Bug 修复
2. 代码格式调整
3. 注释更新
4. 依赖升级（无 Breaking Changes）

### 9.3 文档模板

#### 功能开发文档模板
```markdown
# <功能名称>

> **开发日期**: YYYY年MM月DD日
> **负责人**: [姓名]
> **相关 Issue**: #<issue_number>

## 背景

简述功能开发的背景和需求。

## 功能设计

### 架构设计

### 技术选型

### 接口设计

## 实现细节

### 核心代码

### 关键逻辑

## 测试

### 单元测试

### 集成测试

## 部署

### 配置变更

### 数据库迁移

## 总结

### 遇到的问题

### 经验教训
```

#### 问题修复文档模板
```markdown
# <问题标题>

> **修复日期**: YYYY年MM月DD日
> **问题级别**: P0/P1/P2
> **影响范围**: [描述]

## 问题描述

详细描述问题现象。

## 问题原因

根本原因分析。

## 解决方案

### 方案对比

### 最终方案

## 修复代码

关键代码片段和说明。

## 测试验证

测试步骤和结果。

## 预防措施

如何避免类似问题。
```

### 9.4 文档命名规范

- ✅ **使用中文名称**：准确概括内容
- ✅ **简洁明了**：不超过 20 个汉字
- ✅ **包含关键词**：便于搜索

示例：
- `服务层重构方案.md`
- `Windows点击位置偏差问题修复.md`
- `node-screenshots集成与窗口级截图功能.md`

### 9.5 代码注释规范

```typescript
/**
 * BaseOperateService - 操作服务基类
 *
 * 提供 Web 和 Windows 服务的公共功能：
 * - 状态管理
 * - 回调机制
 * - 生命周期管理
 * - Report 生成和上传
 *
 * @example
 * ```typescript
 * class MyService extends BaseOperateService<MyAgent> {
 *   protected async createAgent() {
 *     // 实现 Agent 创建
 *   }
 * }
 * ```
 */
export abstract class BaseOperateService<TAgent> {
  /**
   * Agent 实例
   * @public
   */
  public agent: TAgent | null = null;
  
  /**
   * 创建 Agent 实例
   * 子类需要实现自己的 Agent 创建逻辑
   * 
   * @abstract
   * @protected
   */
  protected abstract createAgent(): Promise<void>;
}
```

---

## 10. Git 提交规范

### 10.1 提交消息格式

```
<type>(<scope>): <subject>

<body>

<footer>
```

### 10.2 Type 类型

| Type | 说明 | 示例 |
|------|------|------|
| `feat` | 新功能 | `feat(service): 添加 Windows 服务支持` |
| `fix` | Bug 修复 | `fix(websocket): 修复连接断开问题` |
| `refactor` | 重构 | `refactor(service): 提取公共基类` |
| `docs` | 文档 | `docs: 更新开发规范` |
| `style` | 格式 | `style: 统一代码格式` |
| `test` | 测试 | `test(service): 添加单元测试` |
| `chore` | 构建/工具 | `chore: 升级依赖版本` |
| `perf` | 性能优化 | `perf(service): 优化任务执行性能` |

### 10.3 Scope 范围

- `service`: 服务层
- `websocket`: WebSocket 层
- `handler`: Handler 处理
- `util`: 工具函数
- `type`: 类型定义
- `config`: 配置
- `test`: 测试

### 10.4 提交示例

```bash
# ✅ 好的提交
git commit -m "feat(service): 添加 BaseOperateService 抽象基类"
git commit -m "fix(websocket): 修复消息 ID 不一致问题"
git commit -m "refactor(handler): 使用工厂模式重构 Handler"

# ❌ 不好的提交
git commit -m "更新代码"
git commit -m "修复 bug"
git commit -m "WIP"
```

---

## 11. 服务层开发规范

### 11.1 Service 类结构

```typescript
export class MyService extends BaseOperateService<MyAgent> {
  // 1. 静态单例
  private static instance: MyService | null = null;
  
  // 2. 私有构造函数
  private constructor() {
    super();
  }
  
  // 3. 获取实例
  public static getInstance(): MyService {
    if (!MyService.instance) {
      MyService.instance = new MyService();
    }
    return MyService.instance;
  }
  
  // 4. 重置实例（测试用）
  public static resetInstance(): void {
    MyService.instance = null;
  }
  
  // 5. 实现抽象方法
  protected async createAgent(): Promise<void> {
    // 实现
  }
  
  protected async initializeConnection(): Promise<void> {
    // 实现
  }
  
  protected getServiceName(): string {
    return 'MyService';
  }
  
  // 6. 公共方法
  public async execute(params: any): Promise<any> {
    // 实现
  }
}
```

### 11.2 Service 方法规范

```typescript
// ✅ 方法应该单一职责
public async executeTask(task: Task): Promise<Result> {
  // 1. 参数验证
  this.validateTask(task);
  
  // 2. 状态检查
  this.checkServiceState();
  
  // 3. 执行任务
  const result = await this.performTask(task);
  
  // 4. 处理结果
  return this.processResult(result);
}

// ❌ 避免方法过长
public async executeTask(task: Task): Promise<Result> {
  // 300 行代码...
}
```

### 11.3 状态管理

```typescript
// ✅ 使用枚举管理状态
enum ServiceState {
  STOPPED = 'stopped',
  STARTING = 'starting',
  RUNNING = 'running',
  STOPPING = 'stopping',
}

// ✅ 提供状态查询和转换方法
public getState(): ServiceState {
  return this.state;
}

protected setState(newState: ServiceState): void {
  const oldState = this.state;
  this.state = newState;
  this.emit('stateChanged', { oldState, newState });
}
```

---

## 12. 文件组织规范

### 12.1 目录结构

```
apps/server/src/
├── config/              # 配置文件
│   ├── clientTypeActions.ts
│   └── index.ts
├── mastra/              # Mastra 集成
│   ├── agents/
│   ├── tools/
│   └── mcp/
├── middleware/          # 中间件
│   └── logger.ts
├── routes/              # 路由处理
│   ├── modules/
│   └── index.ts
├── services/            # 业务服务
│   ├── base/           # 基础服务
│   │   ├── BaseOperateService.ts
│   │   ├── WebOperateServiceRefactored.ts
│   │   └── __tests__/
│   └── customMidsceneDevice/
├── types/               # 类型定义
│   ├── index.ts
│   ├── websocket.ts
│   └── windowsProtocol.ts
├── utils/               # 工具函数
│   ├── logger.ts
│   ├── error.ts
│   ├── response.ts
│   └── __tests__/
├── websocket/           # WebSocket 处理
│   ├── actions/        # Action 处理器
│   │   ├── base/
│   │   ├── web/
│   │   └── windows/
│   ├── builders/       # 消息构建器
│   ├── handlers/       # 消息处理器
│   └── helpers/        # 辅助函数
└── index.ts            # 入口文件
```

### 12.2 模块导出规范

```typescript
// ✅ 使用 index.ts 集中导出
// src/services/base/index.ts
export { BaseOperateService } from './BaseOperateService';
export { WebOperateServiceRefactored } from './WebOperateServiceRefactored';
export { WindowsOperateServiceRefactored } from './WindowsOperateServiceRefactored';

// ✅ 类型和实现分开导出
export type { TaskTipCallback, TaskError } from './BaseOperateService';
export { OperateServiceState } from './BaseOperateService';
```

### 12.3 Import 顺序

```typescript
// 1. Node.js 内置模块
import { EventEmitter } from 'node:events';
import path from 'node:path';

// 2. 第三方依赖
import type { AgentOverChromeBridge } from '@midscene/web/bridge-mode';
import { Hono } from 'hono';

// 3. 内部模块（使用别名）
import type { WebSocketMessage } from '@/types/websocket';
import { serviceLogger } from '@/utils/logger';
import { createSuccessResponse } from '@/websocket/builders/messageBuilder';

// 4. 相对路径导入
import { helper } from './helper';
import type { LocalType } from './types';
```

---

## 13. 性能优化规范

### 13.1 异步操作

```typescript
// ✅ 使用 Promise.all 并行执行
const [result1, result2, result3] = await Promise.all([
  fetchData1(),
  fetchData2(),
  fetchData3(),
]);

// ❌ 避免串行执行
const result1 = await fetchData1();
const result2 = await fetchData2();
const result3 = await fetchData3();
```

### 13.2 缓存策略

```typescript
// ✅ 实现缓存机制
class ServiceWithCache {
  private cache = new Map<string, any>();
  
  async getData(key: string): Promise<any> {
    if (this.cache.has(key)) {
      return this.cache.get(key);
    }
    
    const data = await fetchData(key);
    this.cache.set(key, data);
    return data;
  }
  
  clearCache(): void {
    this.cache.clear();
  }
}
```

### 13.3 资源管理

```typescript
// ✅ 及时释放资源
class ResourceManager {
  private resources: Resource[] = [];
  
  async cleanup(): Promise<void> {
    for (const resource of this.resources) {
      await resource.close();
    }
    this.resources = [];
  }
}

// ✅ 使用 finally 确保清理
async function processData() {
  const connection = await createConnection();
  try {
    return await connection.query();
  } finally {
    await connection.close();
  }
}
```

### 13.4 性能监控

```typescript
// ✅ 记录关键操作的性能指标
async function executeTaskWithMetrics(task: Task) {
  const startTime = performance.now();
  
  try {
    const result = await executeTask(task);
    const duration = performance.now() - startTime;
    
    serviceLogger.info({
      taskId: task.id,
      duration,
      success: true,
    }, `任务执行成功，耗时 ${duration}ms`);
    
    return result;
  } catch (error) {
    const duration = performance.now() - startTime;
    
    serviceLogger.error({
      taskId: task.id,
      duration,
      success: false,
    }, `任务执行失败，耗时 ${duration}ms`);
    
    throw error;
  }
}
```

---

## 14. 安全规范

### 14.1 环境变量

```typescript
// ✅ 使用环境变量存储敏感信息
const apiKey = process.env.API_KEY;
const dbPassword = process.env.DB_PASSWORD;

// ❌ 不要硬编码敏感信息
const apiKey = 'sk-1234567890abcdef';  // 危险！
```

### 14.2 输入验证

```typescript
// ✅ 验证所有外部输入
function processUserInput(input: unknown): string {
  if (typeof input !== 'string') {
    throw new AppError('Invalid input type', 400);
  }
  
  if (input.length > 1000) {
    throw new AppError('Input too long', 400);
  }
  
  // 清理和验证输入
  return sanitizeInput(input);
}
```

### 14.3 日志脱敏

```typescript
// ✅ 脱敏敏感信息
function sanitizeLogData(data: any) {
  return {
    ...data,
    password: '***',
    apiKey: data.apiKey?.slice(0, 8) + '...',
    token: '***',
  };
}

serviceLogger.info(
  sanitizeLogData(userData),
  '用户登录'
);
```

---

## 附录

### A. 快速参考

#### 常用命令
```bash
# 开发
pnpm dev

# 构建
pnpm build

# 测试
pnpm test
pnpm test:coverage

# 代码质量
pnpm lint
pnpm format
pnpm check
pnpm fix
```

#### 目录映射
```typescript
// tsconfig.json paths
{
  "@/*": ["./src/*"],
  "@/types/*": ["./src/types/*"],
  "@/utils/*": ["./src/utils/*"],
  "@/services/*": ["./src/services/*"]
}
```

### B. 相关文档

- [服务层重构方案](./架构重构/服务层重构方案.md)
- [代码对比与迁移指南](./架构重构/代码对比与迁移指南.md)
- [WebSocket传值说明](./WebSocket传值说明.md)
- [Monorepo使用指南](./Monorepo使用指南.md)

### C. 工具链接

- [Biome 文档](https://biomejs.dev/)
- [Vitest 文档](https://vitest.dev/)
- [TypeScript 文档](https://www.typescriptlang.org/)
- [Hono 文档](https://hono.dev/)
- [Midscene 文档](https://midscenejs.com/)

---

**文档维护**：本文档由团队共同维护，请在进行重大变更时及时更新。

**反馈渠道**：如有问题或建议，请创建 Issue 或联系项目负责人。

**最后更新**：2025年10月24日

